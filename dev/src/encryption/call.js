...
const caller = {
    ...
    mint_token: async function(){
        const myContract = await getContract(NFTENCRYPTION.path, NFTENCRYPTION.name, TOKEN_ADDRESS);
    ...
    mint_EVT: async function(){
        const myContract = await getContract(EVTENCRYPTION.path, EVTENCRYPTION.name, CONTRACT_ADDRESS);
        const contractWithSigner = myContract.connect(wallet);
