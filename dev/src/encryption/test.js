...
(async function(){
    ...
    const TokenDeploy = await deploy(NFTENCRYPTION.path, NFTENCRYPTION.name, [price.toString()])
    ...
    const EVTDeploy = await deploy(EVTENCRYPTION.path, EVTENCRYPTION.name, [TokenDeploy.address])
